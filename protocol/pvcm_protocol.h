...
typedef enum {
	...
	PVCM_OP_DBUS_SIGNAL         = 0x44,
	PVCM_OP_DBUS_EXPOSE         = 0x45,
	PVCM_OP_DBUS_INVOKE         = 0x46,
...
/* DBUS_CALL_RESP (Linux -> MCU: DBus method reply) */
typedef struct {
	uint8_t  op;
	uint8_t  req_id;
...
/* DBUS_EXPOSE (MCU -> Linux: register DBus endpoint) */
typedef struct {
	uint8_t  op;
	uint8_t  endpoint_id;
	char     path[60];
...
/* DBUS_INVOKE (Linux -> MCU: call MCU endpoint) */
typedef struct {
	uint8_t  op;
	uint8_t  invoke_id;
	uint8_t  endpoint_id;
