...
| `docs/testing/` | *How do I test it?* | pvtest harness, running the suite, authoring tests, the test list | Yocto build steps — those stay in meta-pantavisor |
...
- **pvtest**: the integration test framework lives here in `pvtest/`, split by **where a file runs** — `tester/` (`pvtest-run.in`, `utils.in`) runs inside the tester container, `host/` (`test.docker.sh`, `test.native.sh`, `host-common`, `device.txt`, the three READMEs) runs on the workstation or CI runner, `shared/common.in` is sourced by both, and `suites/{local,remote}/` is the test data. `tester/` and `shared/` are installed by CMake under `PANTAVISOR_PVTEST=ON`; `host/` and `suites/` ship as plain files. The deployed tarball layout is flatter than the source tree and does not follow this split. meta-pantavisor only assembles it into the appengine distro, so **a pvtest change is a change here**, and `.github/workflows/onpush-pvtest.yaml` runs the suite against every non-draft PR -- `local` normally, `local` plus `remote` when the change touches the Hub client (`pantahub/`, `updater`, `trestclient`, `ph_logger`, `event/event_rest`), `pvtest/suites/remote/` or the framework (`pvtest/tester/`, `pvtest/shared/`, `pvtest/host/`), which needs the `PH_USER`/`PH_TOKEN` (Pantahub personal token) repository secrets (`PH_PASS` is a deprecated fallback). Authoring rules and the harness architecture are documented here, in `docs/testing/`; the Yocto build that assembles the distro stays in meta-pantavisor's `docs/overview/testing/automated/`. Never source `utils` on the host: it shadows `pvcontrol`/`pventer`/`pvcurl`/`pvtx`.
- **pvtest container fixtures**: the `pv-example-*` containers the suites run against are bitbake recipes in **meta-pantavisor** (`recipes-containers/pv-examples/`), listed in `PV_PVTEST_CONTAINERS` / `PV_PVTEST_CONTAINERS_XCONNECT` in `pantavisor-appengine-distro.bb`. This repo carries no yocto content, so **a new test container is a meta-pantavisor PR alongside the pantavisor PR carrying the test that uses it**. Building one is `bitbake <name>` in a meta-pantavisor build — no `devtool modify`, the recipe does not need pantavisor's source — and `./test.docker.sh install-tarballs <type> <name>.pvrexport.tgz` drops the result into an installed distro without rebuilding it. meta-pantavisor pins this repo once, as recipe source (`PANTAVISOR_SRCREV` in `pantavisor.inc`).
- **pvtest cross-repo work**: what still spans both repos is a change to `container-pvrexport.bbclass`, to the assembly recipes, or to `pv-avahi`/`pv-avahi-browse`. To validate a pair before either merges, dispatch `onpush-pvtest.yaml` with `meta_ref` set to the meta branch.
