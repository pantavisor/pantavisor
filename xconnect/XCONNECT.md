...
- **Note**: This section describes the per-provider bus model. See [Pantavisor-Hosted System Bus](#pantavisor-hosted-system-bus) for the lower-friction alternative where pantavisor itself hosts a shared system bus.
...
The per-provider bus model described under [Mediation Patterns / D-Bus](#d-bus) remains fully supported; this is a second, lower-friction option.
...
- Interface- and member-granular policy is not out of scope: an `allow` entry may narrow to specific interfaces, members and paths, and anything that narrowing cannot express is reachable through a validated raw policy fragment — see [Name-Based D-Bus Requirements and Policy](#name-based-d-bus-requirements-and-policy).
...
- **Daemon hardening**: the hosted daemon runs like any other managed daemon; chrooting it into a minimal directory and dropping it to an unprivileged UID via the generated config's `<user>` element is not done yet.

See [D-Bus Service Activation](#d-bus-service-activation) for starting providers and consumers on demand, and [Name-Based D-Bus Requirements and Policy](#name-based-d-bus-requirements-and-policy) for consumer dependency declarations, role UID pinning and policy narrowing/fragments.

## D-Bus Service Activation

Builds directly on [Pantavisor-Hosted System Bus](#pantavisor-hosted-system-bus); the `owns` declaration introduced there is the prerequisite. Scoped to the hosted system bus only.
...
A provider marks an owned name activatable in its `services.json` export:
...
A consumer marks one or more required names in `names` with `activation.mode: on-owner`:
...
2. xconnect calls Pantavisor:
   ...
   ```
   POST /xconnect/dbus/activate   { "name": "org.example.Foo" }
   ```
...
`POST /xconnect/dbus/activate` accepts exactly one of `name` (a well-known bus name — provider activation) or `container` (a platform name — consumer activation):
...
xconnect opens **one** persistent connection to the hosted daemon at startup, authenticated as itself, with a match rule for `NameOwnerChanged`. That single connection is the ownership oracle for both activation directions: it tells a cold provider-activation session the moment its target name acquires an owner (driving step 4 above), answers "is this name already owned?" when a session first inspects a call (so an already-owned name skips activation entirely), and drives the consumer waiters described in [Consumer Activation](#consumer-activation-on-owner). This connection is the only place xconnect constructs/parses D-Bus method calls and signals; the per-session cold path only needs to *frame* messages and read one header field.

### Dependency Semantics

Pantavisor's activation endpoint does no dependency resolution of its own: it maps a name or container to a platform and promotes it, nothing more. A multi-step chain — a client calls `Foo`, which itself needs `Bar` — still activates end to end, but **emergently**: once `Foo` is promoted and running, `Foo`'s own outbound call to `Bar` is itself a cold call on `Foo`'s proxied connection and re-enters the exact same in-proxy hold path described above. No separate dependency graph is built or walked, and no container's declared required services are inspected by the activation endpoint.

This has a real consequence: there is **no cycle detection**. Two on-demand names that call each other on first use each hold their caller's message waiting for the other's ownership; nothing breaks the cycle proactively — each side's own [async timeout](#async-activation-no-mainloop-blocking) is what eventually fails it with a typed error, not a fast validation-time rejection. Author activatable dependencies as a DAG.
...
- A cold session that triggers provider activation **disables its client read side** and holds the matched message; it does not read or block.
...
- A per-wait **`evtimer`**, set *below* the client's own D-Bus reply timeout (libdbus default ~25s), bounds provider-activation failure. If it fires first, xconnect synthesizes a proper `org.freedesktop.DBus.Error.TimedOut` reply to the client (so the call fails cleanly with a reason, and the client may retry against a now-partially-warm chain) rather than letting the connection hang or dropping it opaquely. A `POST /xconnect/dbus/activate` that itself fails (pv-ctrl unreachable, or a non-2xx response) fails the held call the same way, with `org.freedesktop.DBus.Error.Failed`.
- Consumer (`on-owner`) waiters carry no timer: a failed `POST` just clears the "fired" flag so the next ownership change re-evaluates it, and there is no held call to reply to.

There is no single timer bounding a whole chain and no synchronous wait anywhere: the loop keeps servicing all other links throughout. The only end-to-end ceiling for a provider-activation chain is the client's own reply timeout, which is outside our control; xconnect's `evtimer` deliberately sits under it.
...
- **No idle shutdown / deactivation**, in either direction. Starting on demand is independently useful; deciding *when to stop* a container needs its own policy model and is deferred to a separate effort. Once started, an activated container follows its normal restart policy.

### Status and follow-ups

- **Live reload is `SIGHUP`.** The pre-flight validation described under [Name-Based D-Bus Requirements and Policy](#name-based-d-bus-requirements-and-policy) is the actual enforcement point; switching the live reload itself to `org.freedesktop.DBus.ReloadConfig` is deferred because that call would travel over `pv-xconnect`'s own connection and so cannot fail the state transition inside Pantavisor.
- **No dependency engine.** Cycle detection and true recursive dependency activation are not implemented; chains work only through the emergent re-entry described in [Dependency Semantics](#dependency-semantics).

## Name-Based D-Bus Requirements and Policy
...
2. a role can be **pinned to a real uid** for legacy authorization;
3. providers can **refine the generated bus policy**, declaratively or with a
   validated policy fragment.

Everything here builds on what already exists: `owns`/`allow` exports, the
role-per-socket rule, and the generated default-deny policy. Consumer
passivity via `on-owner` names is covered in
[D-Bus Service Activation](#d-bus-service-activation), which this section's
`names` form feeds.
...
exists but carries no runtime meaning. `names` closes three gaps:
...
- `GetConnectionUnixProcessID` still returns `pv-xconnect`'s pid; masquerading
  it is not implemented (see [Role UID Pinning](#role-uid-pinning)).
