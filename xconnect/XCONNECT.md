...
1. **Spec/docs** (this section): `activation.mode` on both providers and consumers, the passive-container pattern, the in-proxy trigger, the consumer waiter, async activation, dependency semantics, failure modes.
2. **Parser/state model**: add `activation` metadata to `pv_platform_service_export`; build an index from `(bus, owns)` to owner platform; expose the activatable-name set to xconnect (alongside the `uid` already carried on hosted-bus graph entries); parse `on-owner` on a consumer's `names` entries.
...
4. **In-proxy cold/warm state machine + control endpoint**: teach the dbus plugin to frame messages and read `DESTINATION` on the cold path, hold/coalesce/release with the async `evtimer` bound and synthesized error reply; add `POST /xconnect/dbus/activate` accepting `name` or `container` (mirrors the existing `GET /xconnect-graph` client in `main.c`). Revert to splice once warm. Honor `NO_AUTO_START`.
5. **Consumer waiter** (xconnect + Pantavisor): track each passive consumer's `on-owner` names, seed from `ListNames`, fire `{"container": ...}` once all are owned, promote via the same `MOUNTED → STARTED` path as provider activation.
6. **Tests**: passive provider not started at boot; client call activates it transparently; passive consumer starts once its provider owns its name; passive-consumer-on-passive-provider stays idle; `NO_AUTO_START` is not activated; always-on behavior unchanged.

## Name-Based D-Bus Requirements and Policy (Design)
