...
static void wayland_proxy_event_cb(struct bufferevent *bev, short events, void *arg)
{
...
static void wayland_on_accept(struct evconnlistener *listener, evutil_socket_t fd,
                              struct sockaddr *address, int socklen, void *arg)
{
    ...
    bufferevent_setcb(be_client, wayland_proxy_read_cb, NULL, wayland_proxy_event_cb, be_provider);
    bufferevent_setcb(be_provider, wayland_proxy_read_cb, NULL, wayland_proxy_event_cb, be_client);
...
static int wayland_on_link_added(struct pvx_link *link)
{
    ...
    link->listener = evconnlistener_new_bind(base, wayland_on_accept, link,
        LEV_OPT_CLOSE_ON_FREE | LEV_OPT_REUSEABLE, -1, fd);

    return 0;
}

struct pvx_plugin pvx_plugin_wayland = {
    .type = "wayland",
