...
static void rest_read_client_cb(struct bufferevent *bev, void *arg)
{
    ...
    if (!session->headers_injected) {
        char headers[512];
        snprintf(headers, sizeof(headers), "X-PV-Client: %s\r\nX-PV-Role: %s\r\n", 
                           ...
                           struct sockaddr *address, int socklen, void *arg)
{
    ...
    if (bufferevent_socket_connect(session->be_provider, (struct sockaddr *)&sun, sizeof(sun)) < 0) {
    ...
    bufferevent_setcb(session->be_client, rest_read_client_cb, NULL, proxy_event_cb, session);
    bufferevent_setcb(session->be_provider, rest_read_provider_cb, NULL, proxy_event_cb, session);
...
static int rest_on_link_added(struct pvx_link *link)
{
    ...
    link->listener = evconnlistener_new(base, rest_on_accept, link,
...
struct pvx_plugin pvx_plugin_rest = {
    .type = "rest",
