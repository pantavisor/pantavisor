...

{
	...
	for (int i = 0; items[i]; i++) {
		...
		if (p) {
			...
			char *consumer_tmp = pv_json_get_value(

				json, "consumer", itok, obj_tokc);
			...
			link->role = pv_json_get_value(json, "role", itok,

						       obj_tokc);

			link->provider_socket =

				pv_json_get_value(json, "socket", itok,

						  obj_tokc);

			link->interface = pv_json_get_value(

				json, "interface", itok, obj_tokc);
			...
			char *pid_str = pv_json_get_value(
				...
				json, "consumer_pid", itok, obj_tokc);
