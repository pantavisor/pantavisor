...
static void reconcile_graph(const char *json)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (jsmnutil_parse_json(obj_s, &ov, &obj_c) > 0) {
            char *type = pv_json_get_value(obj_s, "type", ov, obj_c);
            struct pvx_plugin *p = find_plugin(type);
            
            if (p) {
                struct pvx_link *link = calloc(1, sizeof(*link));
                link->type = type;
                link->name = pv_json_get_value(obj_s, "name", ov, obj_c);
                link->consumer = pv_json_get_value(obj_s, "consumer", ov, obj_c);
                link->role = pv_json_get_value(obj_s, "role", ov, obj_c);
                link->provider_socket = pv_json_get_value(obj_s, "socket", ov, obj_c);
                
                // For host testing, create a virtual path
                char path[1024];
                snprintf(path, sizeof(path), "/tmp/pvx_%s_%s.sock", link->consumer, link->name);
                ...
                printf("Adding link: %s (%s) -> %s\n", link->consumer, link->type, link->provider_socket);
