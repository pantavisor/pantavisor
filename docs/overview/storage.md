...
Check the [list of default key-value pairs](../../../reference/legacy/pantavisor-metadata.md#device-metadata) that Pantavisor sends.
...
Check out the [list of default user metadata](../../../reference/legacy/pantavisor-metadata.md#user-metadata) consumed by Pantavisor.

## Config

Stores the `PH_` prefixed keys of the [configuration](../../../reference/legacy/pantavisor-configuration.md#summary). This includes the [Pantacor Hub](remote-control.md#pantacor-hub) client credentials.
...
Additional volumes can be [specified](../../../reference/legacy/pantavisor-state-format-v2.md#devicejson) to be used internally by Pantavisor. These are some special cases that can be configured as volumes:
...
Pantavisor can centralize all of your [container logs](containers.md#loggers), separated by [revision](revisions.md), in one place on-disk. It does so by running a small server (Log Server) which [offers](containers.md#roles) a couple of [sockets](../../../reference/legacy/logserver-sockets.md) to the [containers](containers.md):
...
All the log outputs, except for the NULL Sink, will print the timestamp along the log lines. It is possible to configure the format of the timestamp with the [configuration](../../../reference/legacy/pantavisor-configuration.md#summary) key `log.<output>.timestamp.format`, which can be set with the following values:
...
As running a full SHA hashsum of all artifacts can be time consuming for low spec targets and thus increase boot up time, a lazy integrity validation on [container storage](containers.md#storage) can be performed using handlers, which are specified at the [state JSON](../../../reference/legacy/pantavisor-state-format-v2.md#containerrunjson). Handlers are scripts to mount/unmount volumes. The only supported handler so far is [dm-verity](https://www.kernel.org/doc/html/latest/admin-guide/device-mapper/verity.html).

Checksum can also be totally disabled at the [configuration level](../../../reference/legacy/pantavisor-configuration.md#summary) if boot up speed needs to be boosted further, with the `PV_SECUREBOOT_CHECKSUM` key.

#### State Signature

Additionally to artifact checksum, the [state JSON](../../../reference/legacy/pantavisor-state-format-v2.md) can be signed. This can be done from your host computer or from an automated CI, and it is then validated from Pantavisor using a cryptographic hash algorithm. It can be [tuned](../../../reference/legacy/pantavisor-configuration.md#summary) according to one of these three levels of severity:
...
* when a new [remote update](remote-control.md#pantacor-hub-client) is received or a new [local update](local-control.md#local-control) is [run](../../../reference/legacy/pantavisor-commands.md#commands). In case of failure, the update will be reported as [WONTGO](updates.md).

In either case, Pantavisor will parse all [signatures](../../../reference/legacy/pantavisor-state-format-v2.md#_sigscontainerjson), parse the [protected JSON](../../../reference/legacy/pantavisor-state-format-v2.md#protected), calculate the hash depending on the elements included in the JSON and compare them with the signatures.
...
In this option, Pantavisor will need the RSA public key to be stored on disk. You can store it during [build time](../../../environment-setup.md) from the [ca/ directory](../../../reference/legacy/customize-build-pantavisor.md#ca).

It will need the signature and information about how to generate it in the [revision checkout](../../../reference/legacy/pantavisor-state-format-v2.md#_sigscontainerjson). We offer [pvr sig](https://docs.pantahub.com/pvr/#pvr-sig-commands) for convenience.

##### Certificate chain of trust

In this case, the public key will travel in the [revision checkout](../../../reference/legacy/pantavisor-state-format-v2.md#_sigscontainerjson) in a x509 certificate. The certificate will be validated by Pantavisor against a root certificate located on disk .

The root certificate is placed during [Pantavisor build](../../../environment-setup.md). The build system will extract the files from the [ca/ directory](../../../reference/legacy/customize-build-pantavisor.md#ca).

Depending on the subject CN of the signature certificate, we validate it with the following criteria:

* If the subject CN is equal to the value set in `PV_OEM_NAME` from your [configuration](../../../reference/legacy/pantavisor-configuration.md) and the signature does not [includes](../../../reference/legacy/pantavisor-state-format-v2.md#protected) any artifact in the [BSP](../../../reference/legacy/pantavisor-state-format-v2.md#spec-format) or [OEM configuration](../../../reference/legacy/pantavisor-state-format-v2.md#oem-configuration) directories, as those are can only be secured by the default root certificate, we validate it against the OEM root certificate.
* For every other case, the signature is validated against the default root certificate.

It will also need the signature and information about how to generate it in the [revision checkout](../../../reference/legacy/pantavisor-state-format-v2.md#_sigsjson). For this, you can use [pvr sig](https://docs.pantahub.com/pvr/#pvr-sig-commands) command.
...
If a [remote update](remote-control.md) requires more disk space than available, the garbage collector will be activated. This can be adjusted with the configuration [PV_STORAGE_GC_RESERVED parameter](../../../reference/legacy/pantavisor-configuration.md#summary).
...
Disk usage will be checked periodically and garbage collector will be activated if that threshold is reached. By default it is disabled. This can be changed with the [PV_STORAGE_GC_THRESHOLD parameter](../../../reference/legacy/pantavisor-configuration.md#summary).

If an object is put using the [objects endpoint](../../../reference/legacy/pantavisor-commands.md#objects), the threshold will be disabled temporarily, which is done to avoid removing objects that could be linked to the upcoming new revision. This parameter can be changed from the [configuration](../../../reference/legacy/pantavisor-configuration.md#summary).

### On Demand

Finally, our garbage collector can be triggered on demand issuing a [command](../../../reference/legacy/pantavisor-commands.md#commands) through Pantavisor control socket.
